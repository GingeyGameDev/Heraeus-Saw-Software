...
def stopVibrationSensor(startTime):
  vibStopTime = VibrationSensor.stopTiming(startTime)
  return vibStopTime
...
while True:
  ...
  while RFIDScannedIn.on:
      ...
      vibTotalTime = stopVibrationSensor(vibStartTime)
      justRan = True
  if justRan:
    SheetsAPI.libreUpdate([limitSwitchTotalTime, vibTotalTime])
